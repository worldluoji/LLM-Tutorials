...
- `--transport http`（默认，即 Streamable HTTP）：用于连接远程的、通过 HTTP 提供服务的 MCP 服务器。当需要流式响应时服务端会切到 SSE 通道推送事件，是 2025 年规范推荐的远程传输。
- `--transport sse`（已废弃）：用于连接老的、纯 Server-Sent Events 长连接 MCP 服务器。新部署请勿再使用；存量服务可参考 MCP 官方迁移指南升级到 Streamable HTTP。
