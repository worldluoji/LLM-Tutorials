...
loop.ts 根本不关心 bash 工具是怎么运行的，也不关心 Claude 模型的 HTTP 请求怎么发。
它只负责维护这根脆弱但重要的“上下文时间线”（contextHistory）。它像一个忠实的书记员，严格执行了 ReAct 范式：把模型的意图（ToolCall）交给执行层，再把物理世界的反馈（Observation）原封不动地追加回内存中。


第 4 步： Mock测试
```bash
pnpm exec tsx test/engine/test.one-stage.engine.ts
```
你将清晰地看到 Main Loop 在终端中完美地驱动了两个 Turn 的循环,虽然我们接入的还是“假肢”，但这个基于 ReAct 范式的微型操作系统“心脏”，已经确确实实、稳定地跳动起来了！