...
> **普林斯顿大学 SWE-Agent 论文**（2024 年发表，使用 GPT-4 时代的模型）：
>
> **同一个 GPT-4 模型**，只改了 Harness 的接口设计（没有换模型！），性能从 **3.97%** 提升到 **12.47%**，**相对提升 64%**！
>
> *注：以上数据为 GPT-4 时代基线。进入 Claude Sonnet 4.6 / GPT-5 时代后，同类 SWE-Bench 上的绝对分数已大幅提升（领先模型可达 60%+），但 Harness 对同一模型的相对增益依然显著。*
