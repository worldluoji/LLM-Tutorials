...
当用户发送 prompt（例如：我要查询北京的天气） 到 MCP Hosts 时，MCP Hosts 会调用 MCP Client 与 MCP Server 进行通信，获取当前 MCP Server 具备哪些能力;然后,MCP Host 将之前获取到的 MCP Server 的能力，连同用户的 prompt 一起发送给大模型，大模型就可以针对用户的提问，决定何时使用这些能力了。这个过程就类似，我们填充 ReAct 模板，发送给大模型。
...
基于MCP架构设计的核心原则​（解耦能力执行与智能决策，由协调中枢管理会话并衔接LLM与工具）：
- 在标准、典型的MCP架构中，与大模型（LLM）进行交互的核心组件始终是 MCP Hosts。​​ MCP Hosts 承担了与大模型“对话”、传递上下文信息、接收决策指令、反馈执行结果的核心责任。
- 其他组件（Server, Client, Tools）的核心职责聚焦在能力的注册、管理和执行上，并不直接参与与大模型的智能决策对话流程。
