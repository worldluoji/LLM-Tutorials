...
## 补充：理解Sampling的意义
单看"调用 LLM"这一步，Sampling 确实绕了一圈。但 Sampling
解决的不是"调用效率"，而是服务器凭什么能调用 LLM ——
也就是凭证、模型选择权和数据流向。

核心原因有四层，按重要性排：                                                    
  
  1. 凭证与计费平面：社区/第三方 MCP 服务器没有你的 API Key，也不应该有。Sampling 
  让服务器"借"用户的模型订阅来用，否则每个 MCP                                  
  工具都得自己掏钱买账号——没人会开发这种工具。

  2. 模型选择权在用户手里：你在 Claude Code 里选了 Sonnet                       
  4.6，谁都不想某个工具背后偷偷换成便宜模型或别的厂商。Sampling 里服务器只能提    
  modelPreferences.hints（软提示），最终选哪个模型由 host 决定。

  3. 隐私边界：服务器不必持有你上传给 LLM 的原文。host                            
  可以先脱敏、再注入上下文（includeContext: "thisServer" / "allServers"           
  管的就是这个）。

  4. Human-in-the-loop 不是装饰：host 在真正调 LLM 之前可以弹窗让你看             
  prompt、改参数、拒绝。对审计、合规（医疗/金融）、"这个工具到底在让 AI           
  干什么"的透明度都关键。
                                                                                 
最典型的场景：装一个社区的「PDF 合同摘要」MCP 工具。它要总结你的合同——它没你的  
API Key、不该看到合同原文、更不该替你决定用哪个模型。Sampling
就是这三件事的标准答案：服务器说"请用你的模型总结你给我的这份 PDF"，host        
弹窗让你确认，全程数据不外泄。      