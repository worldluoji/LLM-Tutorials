...
```ts
// A2A v1.0 状态枚举（0.x 中的 "canceled"/"unknown" 已分别更正为 "cancelled"/移除）
type TaskState =
  | "submitted"
  | "working"
  | "input-required"
...
> **v1.0 关键变化**：0.x 草案中的 `tasks/sendSubscribe` / `tasks/resubscribe` RPC 在 v1.0 中被合并为基于 Message 的 `message/stream` 流式接口（见下方 `SendStreamingMessageRequest`）。
```ts
// sent by server during message/stream requests (v1.0; 0.x 中名为 sendSubscribe)
...
- server.py(或者__main__.py等)：服务器启动入口。
...
## 示例
示例地址：https://github.com/a2aproject/A2A/blob/main/samples/python/agents/langgraph/README.md
...
学习示例：
https://github.com/worldluoji/a2a-in-action

---

## references
- https://github.com/a2aproject/A2A
- https://a2aproject.github.io/A2A/#/documentation?id=agent-card
- https://a2aproject.github.io/A2A/#/documentation?id=core-objects