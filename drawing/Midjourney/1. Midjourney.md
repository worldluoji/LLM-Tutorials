...
从 22 年 11 月的 Midjourney v4、Nijijourney，到 v5 / v5.2 / v6 / v7 系列，MJ 凭借其高质量的生成效果、分档订阅服务（Basic / Standard / Pro / Mega 等），实现了非常可观的收入。因此，训练出对标 Midjourney 的模型也成为了很多大企业追求的目标。

> **2026 提示**：v7 已是 Midjourney 当前默认版本，`--v 5` / `--v 5.2` / `--niji 5` 已逐渐被官方降权甚至下线；多角色一致性推荐使用 `--cref` + `--cw`；多主体参考可叠加 `--oref` / `--ow`（参考下文 7. oref.md）。
