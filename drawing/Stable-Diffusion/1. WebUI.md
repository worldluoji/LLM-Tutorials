...
- Stable Diffusion checkpoint：这里可以选择已经下载的模型。目前许多平台支持开源的 SD 模型下载，例如 Civitai、Hugging Face 等。
  > 注：Civitai 在 2024-2025 年经历了支付风波与商业化转型，部分模型已迁移至 Tensor.art 等替代平台；模型链接的稳定性建议二次确认。
...
> 原生 `AUTOMATIC1111/stable-diffusion-webui` 仓库已显著放缓迭代，2026 主流已转向以下分支与全新架构：

### A1111 生态下的活跃分支

| 项目 | 特点 | 推荐场景 |
| :--- | :--- | :--- |
| **Forge** (lllyasviel/forge) | A1111 的激进优化分支，内存占用更低、速度更快，原生支持 SDXL / Flux | 想保留 A1111 插件生态、追求性能 |
| **reForge** | Forge 的进一步精简版，砍掉冗余扩展 | 低显存机器首选 |
| **SD.Next** (vladmandic/sdnext) | 多分支合并的现代化 WebUI，支持 SD1.5 / SDXL / SD3 / Flux 全家桶 | 想一个 UI 跑所有主流模型 |

### 新一代架构（不再基于 UNet+CLIP）

- **SD3 / SD3.5**（Stability AI）：基于 DiT（Diffusion Transformer）+ MMDiT，文本理解与排版能力大幅提升；推荐通过 ComfyUI 或 SD.Next 调用。
- **Flux.1**（Black Forest Labs）：12B 参数 DiT 模型，文字渲染与写实风格领先；生态迅速扩张，ComfyUI 原生节点最完善。
- **Wan / Qwen-Image / HunyuanImage** 等国产 DiT：中文 prompt 友好，开源版本可在 16-24GB 显存上运行，推荐 ComfyUI 工作流。

**建议**：新项目直接从 **ComfyUI + SD.Next** 入手；老 A1111 用户迁移到 **Forge** 几乎零成本。