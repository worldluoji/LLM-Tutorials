# 如何用 LLM 帮助写代码（DeepSeek / Cursor / Claude Code）
- 第一种是直接在 DeepSeek 等大模型的问答网页前端输入提示词生成代码，这种最简单，属于体验级的代码生成；
- 第二种则是使用 Cursor 等代码生成工具；
- 第三种则是利用Claude Code 等 cli。

---

## 一. 直接在 DeepSeek 等大模型的问答网页前端输入提示词生成代码
### 1. 提出需求
...
- 可以使用“测试驱动开发”的方式，让 GPT 帮忙写代码，提前让 GPT 写好测试用例。

---

## 二. 使用 Cursor 等代码生成工具
直接使用cursor，让 大模型 直接在代码编辑器里写代码，然后提交代码。

优点：
- 专业工具，支持Claude，GPT，DeepSeek等大模型。
- 快速，代码生成速度非常快。

缺点：
- 需要互联网
- 需要付费，Cursor 当前为多档订阅：Free / Pro（$20/月）/ Business（$40/用户/月）/ Ultra（$200/月），具体价格以官网为准。

---

## 三. 利用Claude Code 等 cli
详见： catalyst/AIWorkFlow