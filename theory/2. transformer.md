...
总之，编码器负责对输入序列进行抽象表示，解码器根据这些表示构建合适的输出序列。以机器翻译为例，Encoder（编码器）负责理解输入句子（例如“Hello, world!”），提取语义特征。Decoder（解码器）则负责根据编码结果，逐词生成目标语言（翻译为“你好，世界！”）。
...
如上图，颜色越深表示单词间相关度越高。那么怎样才能将每个单词的信息按不同权重整合起来呢 ？
